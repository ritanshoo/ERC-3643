...
const { deployTokenProxy } = require('./helpers/tokenProxy');
...
contract('Token', (accounts) => {
  ...
  beforeEach(async () => {
    gasAverage = await fetch('https://ethgasstation.info/json/ethgasAPI.json').then((resp) => resp.json()).then((data) => data.average);
    ...
    identityRegistry = await IdentityRegistry.new(trustedIssuersRegistry.address, claimTopicsRegistry.address, identityRegistryStorage.address, {from: tokeny});
    tokenOnchainID = await deployIdentityProxy(tokeny);
    tokenName = 'TREXsssDINO';
    tokenSymbol = 'TREsX';
    tokenDecimals = '0';
    ...
    */
    let data = {
      _implementationAuthority: implementation.address,
      _identityRegistry: identityRegistry.address,
      _compliance: defaultCompliance.address,
      _name: tokenName,
      _symbol: tokenSymbol,
      _decimals: web3.utils.BN(tokenDecimals),
      _onchainID: tokenOnchainID.address
    }

    token = await deployTokenProxy(data,tokeny);
    console.log({token})
    // console.log(data)
    // const {_compliance,_onchainID,_symbol,_decimals,_name,_implementationAuthority,_identityRegistry} = data
